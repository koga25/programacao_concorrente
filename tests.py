...
json_file = open("./tcp/test.json", 'r')
tcp_rtts = json.load(json_file)
json_file.close()

json_file = open("./udp/test.json", 'r')
udp_rtts = json.load(json_file)
json_file.close()

mean_tcp_rtt = sum(tcp_rtts) / len(tcp_rtts)
mean_udp_rtt = sum(udp_rtts) / len(udp_rtts)

x = np.arange(len(concurrent_clients_tests))  # the label locations
width = 0.35  # the width of the bars
  
tcp_bar = plt.bar(x - width/2, mean_tcp_rtt, width, label = 'TCP')
udp_bar = plt.bar(x + width/2, mean_udp_rtt, width, label = 'UDP')
